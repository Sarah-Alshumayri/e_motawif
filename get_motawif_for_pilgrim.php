...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("SELECT u.name, u.user_id FROM motawif_pilgrim mp
                            JOIN users u ON mp.motawif_id = u.id
                            JOIN users p ON mp.pilgrim_id = p.id
                            WHERE p.user_id = ?");
    $stmt->bind_param("s", $pilgrim_user_id);
    $stmt->execute();
    $stmt->bind_result($motawifName, $motawifUserId);

    if ($stmt->fetch()) {
        echo json_encode([
          'success' => true,
          'motawif_name' => $motawifName,
          'motawif_id' => $motawifUserId
        ]);
    } else {
